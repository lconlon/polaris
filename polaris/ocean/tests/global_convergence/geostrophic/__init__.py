...
from polaris.ocean.tests.global_convergence.geostrophic.analysis import (
    Analysis,
)
from polaris.ocean.tests.global_convergence.geostrophic.forward import Forward
from polaris.ocean.tests.global_convergence.geostrophic.init import Init
from polaris.ocean.tests.global_convergence.geostrophic.viz import Viz, VizMap
from polaris.validate import compare_variables


class Geostrophic(TestCase):
    """
    ...
    def __init__(self, test_group, icosahedral):
        ...
        Parameters
        ----------
        test_group : polaris.ocean.tests.geostrophic.GlobalOcean
        ...
        """
        if icosahedral:
            subdir = 'icos/geostrophic'
        else:
            subdir = 'qu/geostrophic'
        super().__init__(test_group=test_group, name='geostrophic',
                         subdir=subdir)
        ...
        package = 'polaris.ocean.tests.global_convergence.geostrophic'
        config.add_from_package(package, 'geostrophic.cfg')
...
#                              filename1=f'{mesh_name}/forward/output.nc')
    ...
    def _setup_steps(self, config):
        ...
        config.set('geostrophic', 'resolutions', default_resolutions,
                   comment='a list of resolutions (km) to test')

        # get the resolutions back, perhaps with values set in the user's
        # config file, which t priority over what we just set above
        resolutions = config.getlist('geostrophic', 'resolutions', dtype=int)
